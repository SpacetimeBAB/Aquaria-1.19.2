...
public class LatimeriaEntity extends AbstractSchoolingFish implements IAnimatable, Bucketable {
	
    public LatimeriaEntity(EntityType<? extends AbstractSchoolingFish> p_27523_, Level p_27524_) {
    ...
    private static final EntityDataAccessor<Integer> DATA_ID_TYPE_VARIANT=
            SynchedEntityData.defineId(LatimeriaEntity.class, EntityDataSerializers.INT);

    public static AttributeSupplier.Builder attributes() {
        return Mob.createMobAttributes()
                .add(Attributes.MAX_HEALTH, 15.0D)
                .add(Attributes.MOVEMENT_SPEED, (double) 0.75D)
	...
	private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
		if (this.isInWater() && event.isMoving()) {
			event.getController()
					.setAnimation(new AnimationBuilder().addAnimation("animation.latimeria.swim", true));
			return PlayState.CONTINUE;
		}
		if (!this.isInWater()) {
			event.getController()
					.setAnimation(new AnimationBuilder().addAnimation("animation.latimeria.flop", true));
			return PlayState.CONTINUE;
		}
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor p_146746_, DifficultyInstance p_146747_,
    	...
    	LatimeriaVariant variant = Util.getRandom(LatimeriaVariant.values(), this.random);
    	setVariant(variant);
    	return super.finalizeSpawn(p_146746_, p_146747_, p_146748_, p_146749_, p_146750_);
    }

    public LatimeriaVariant getVariant() {
    	return LatimeriaVariant.byId(this.getTypeVariant() & 255);
    }
    ...
    private void setVariant(LatimeriaVariant variant) {
